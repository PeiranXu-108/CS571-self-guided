...
function calculateSubtotal() {
	let res = 0;
	const rows = document.getElementById("price-board").getElementsByTagName("tr");
	console.log(rows)
	for (let i = 1; i < rows.length; i++) {
		let item = rows[i].getElementsByTagName("td")[0].innerText.toLowerCase().slice(0, -1);
		console.log(item);
	...
	return res; // TODO calculateSubtotal
}

function calculateSalesTax() {
	const state = document.getElementById("state-tax").value;
	console.log(state)
	return getSalesTaxRateForState(state) * calculateSubtotal(); // TODO calculateSalesTax
}
